<template>
  <div class="container mt-4">
    ...
    <form @submit.prevent="actualizarProveedor">
      ...
      <router-link to="/proveedores" class="btn btn-outline-secondary ms-2"
...
export default {
  ...
  methods: {
    ...
    async actualizarProveedor() {
      const id = this.$route.params.id // Obtener el ID desde la ruta
      try {
        const response = await axios.put(`/api/proveedores/${id}`, {
        ...
        const message =
          ...
          'No se pudo actualizar el proveedor. Verifica los datos enviados.'
